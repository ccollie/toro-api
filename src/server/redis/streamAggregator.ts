...
export class RedisStreamAggregator {
  ...
  constructor(options: Partial<RedisStreamAggregatorOptions> = defaultOptions) {
   ...
   * @param onEvent {Function} handler for stream messages
   ...
   * @param onEvent {Function} a previously registered handler for the stream
   */
  unsubscribe(id: string, onEvent): void {
  ...
  async readStream(): Promise<void> {
    ...
    if (messages) {
      ...
      for (let i = 0; i < messages.length; i++) {
        ...
        if (info) {
          const deserialize = info.deserialize || this.options.deserialize;
          const eventMessagesRaw = messages[i][1];
          const eventMessages = eventMessagesRaw.map((r) => {
            const data = deserialize(r[1]);
            return [r[0], data];
          });
