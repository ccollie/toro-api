...
local function filter(obj, predicate)
    assert(isObject(obj), 'expected an array in map')

    local accumulator = {}

    for _, current in ipairs(obj) do
        if (predicate(current, _)) then
...
--
Predicates['$elemMatch'] = function(a, b)
    ---should return false for non-matching input
    if (isArray(a) and (#a > 0)) then
        local format = function (x) return x end
        if (not isObject(b)) then return false end

        local criteria = b
        local allOperators = true
        for op, v in pairs(criteria) do
        ...
        local query = parseExpression(criteria)
        for _, v in ipairs(a) do
            if (query(format(v))) then
                return true
            end
        end
    end
...
Predicates['$startsWith'] = function(haystack, needle)
    ...
    local plen = #needle
    return (#haystack >= plen) and (haystack:sub(1, plen) == needle)
end
...
local function compileQuery(criteria)
    ...
    local function parse(criteria)
        for field, expr in pairs(criteria) do
            ...
            elseif (field == '$and' or field == '$or' or field == '$nor' or field == '$xor') then
                processOperator(field, field, expr)
            else
...
QueryOperators['$xor'] = function(selector, value)
    assert(isArray(value), '$xor expects an array')
    assert(#value == 2, '$xor expects an array(2)')
    local left = compileQuery(value[1])
    local right = compileQuery(value[2])
...
ExprOperators['$xor'] = function(expr)
    ...
    return function(obj)
        local args = exec(obj)
        assert(isArray(args) and #args == 2, '$xor expects an array(2)')
        local a = args[1]
        local b = args[2]
        return a ~= b
    end
...
ExprOperators['$max'] = function(expr)
    return extrema('$max', expr, function(x, y)
        return x > y
    end)
end

ExprOperators['$min'] = function(expr)
    return extrema('$min', expr, function(x, y)
        return x < y
    end)
end
