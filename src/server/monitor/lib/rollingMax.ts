...
export class RollingMax {
  ...
  add(value) {
    ...
    while (!list.isEmpty() && lowerThan(list.peekBack().value, value)) {
      list.pop();
    }

    // In case that all elements in the this.list are all greater than the
    // current one (descending order). Shift out the
    if (!list.isEmpty() && list.peekFront().i < i - windowSize + 1) {
      list.shift();
    }
