    ...
    SerializableSampling
{
  ...
  /**
   * Getter for sum property extracting the string representation from
   * internal sum property (64bit number).
   *
   * @readonly
   * @type {string}
   * @memberof Histogram
  ...
  public update(value: number): this {
    this.count++;
    this.sumInternal = this.sumInternal + BigInt(value);
    for (const boundary of this.bucketsInternal.boundaries) {
      if (value < boundary) {
  ...
  /**
   * Gets the bucket's config object.
   *
   * @returns {Buckets}
   * @memberof Histogram
   */
