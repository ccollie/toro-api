...
describe('timeseries', () => {
  ...
  describe('bulkAdd', () => {
    ...
    it('should add values to the set', async () => {
      const startTs = 1488823384;
      const data = [];

      for (let i = 0; i < 20; i++) {
        data.push(startTs + i, generateRecord());
      }
  ...
  describe('set', () => {
    ...
    it('should create the value if it does not exist', async () => {
      ...
      await set(startTs, data);
      const actual = await getObjectValue(KEY, startTs);
      expect(actual).toEqual(data);
    });
    ...
    it('should set the values', async () => {
      ...
      await insertData(KEY, startTs, data);

      await set(startTs, newValues);
  ...
  describe('updateJson', () => {
    ...
    it('should create the value if it does not exist', async () => {
      ...
      const actual = await update(startTs, data);
      expect(actual).toEqual(data);
    });
    ...
    it('updates the value', async () => {
      ...
      await insertData(KEY, startTs, original);
  ...
  describe('del', () => {
    ...
    it('allows a variable amount of keys', async () => {
      ...
      await insertMany(client, TIMESERIES_KEY, startTs, sampleSize, data);
  ...
  describe('range', () => {
    ...
    function insertData(startTs, sampleSize, value) {
      return insertMany(client, KEY, startTs, sampleSize, value);
    }
    ...
    it('should support getting all values', async () => {
      ...
      await insertData(startTs, sampleSize, data);
  ...
  describe('revrange', () => {
    ...
    it('should support getting all values', async () => {
      ...
      await insertData(startTs, sampleSize, data);
      const response = await get_range('+', '-');
      const actual = response.map((x) => parseInt(x.data, 10));
    ...
    describe('supports special range syntax', () => {
      ...
      test.each(cases)(
        ...
        async (max, min, expected: any[]) => {
          await insertData(startTs, data.length, data);
  ...
  describe('remrange', () => {
    ...
    it('should remove data based on range', async () => {
      ...
      await insertMany(client, KEY, startTs, data.length, data);

      const mid = data.length / 2;
      const midTs = startTs + mid;
      const endTs = startTs + data.length;

      const count = await callMethod(KEY, 'remrange', midTs, endTs);
      ...
      expect(interval[0]).toEqual(startTs);
      expect(interval[1]).toEqual(startTs + mid - 1);
    });
...
async function insertMany(client, key, startTs, sampleSize, data) {
  /***
  insert data to key, starting from startTs, with 1 sec interval between
  them
  @param key: names of time_series
  @param startTs: beginning of time series
  @param sampleSize: number of samples
  @param data: could be a list of sampleSize values, or one value. if a list, insert the
   values in their order, if not, insert the single value for all the timestamps
   **/
  const pipeline = client.pipeline();
  for (let i = 0; i < sampleSize; i++) {
    const value = Array.isArray(data) ? data[i] : data;
    const serialized =
      typeof value === 'object' ? JSON.stringify(value) : value;
    pipeline.timeseries(key, 'add', startTs + i, serialized);
  }
