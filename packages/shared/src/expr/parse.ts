...
function toRpn(_node: jsep.Expression, context: any[]) {
  ...
  function push<TNode extends RpnNode>(data: TNode) {
    context.push(data);
  }
  ...
  function conditional(node: jsep.ConditionalExpression) {
    ...
    const elseOfs = context.length - 1;
    toRpn(node.alternate, context);
    push({ type: 'conditional', elseOfs });
  }
...
function optimizeRPN(rpn: RpnNode[]): RpnNode[] {
  // collapse multiple member calls to single identifier node
...
function optimizeIdentifiers(rpn: RpnNode[]): RpnNode[] {
  ...
  while (i < rpn.length) {
    ...
    if (node.type === 'identifier') {
      let j = i + 1;
      const idParts = [(node as IdentifierNode).name];
