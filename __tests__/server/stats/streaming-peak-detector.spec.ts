...
describe('StreamingPeakDetector', () => {
  describe('constructor', () => {
    it('constructs an instance', () => {
      const sut = new StreamingPeakDetector(systemClock, 1000, 3.5, 0);
  ...
  describe('.isInLagPeriod', () => {
    it('calculates correctly', () => {
      ...
      const sut = new StreamingPeakDetector(clock, 1000, 3.5, 0);
      for (let i = 0; i < 10; i++) {
  ...
  describe('.update', () => {
    it('does not signal on a single value', () => {
      const clock = new ManualClock();
      const sut = new StreamingPeakDetector(clock, 0, 3.5, 0);
    ...
    it('calculates peaks', () => {
      ...
      const sut = new StreamingPeakDetector(clock, 30, 5, 0);
      const actual: number[] = [];
